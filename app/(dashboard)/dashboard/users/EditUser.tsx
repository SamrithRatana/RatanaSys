...
// ─── Google SVG (inline, no extra dep) ───────────────────────────────────────
...
const EditUser = ({ user }: EditUserProps) => {
  ...
  async function onSubmit(values: FormValues) {
    const id = user.id;
    // strip confirmPassword — server doesn't need it
                        ...
                        {orgDepartments.map((dpt) => (
                          ...
                          >
                            <BsCheckLg
                              className={cn(
                                "mr-2 h-4 w-4",
                                dpt.label === field.value ? "opacity-100" : "opacity-0"
                              )}
                            />
                            {dpt.label}
                          </CommandItem>
                        ...
                        {orgTitles.map((title) => (
                          ...
                          >
                            <BsCheckLg
                              className={cn(
                                "mr-2 h-4 w-4",
                                title.label === field.value ? "opacity-100" : "opacity-0"
                              )}
                            />
                            {title.label}
                          </CommandItem>
                      ...
                      <CommandGroup>
                       {(UserRoles as unknown as string[]).map((role, i) => (
                          <CommandItem
                            ...
                            key={i}
                            onSelect={() =>
                              form.setValue("role", role as FormValues["role"])
                            }
                          >
                            <BsCheckLg
                              className={cn(
